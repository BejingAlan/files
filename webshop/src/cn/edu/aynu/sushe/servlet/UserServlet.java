...
public class UserServlet extends HttpServlet {
	...
	public void register(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 3、注册，如果发生异常，就到register.jsp页面上显示：用户名已存在；
		 *         如果成功，就在index.jsp页面显示：注册成功
		 */
	...
	public void login(HttpServletRequest request, HttpServletResponse response)
		 ...
		 *        如果成功：就将查询到到的User对象u，存放到session域中，
		 *        然后转发到shopping.jsp页面,显示：欢迎用户  登录
		 */
		User user = CommonsUtils.toBean(request.getParameterMap(), User.class);
		...
		try {
			...
			request.getRequestDispatcher("/ProductServlet?method=1").forward(request, response);
			}else {
				request.setAttribute("msg","验证码错误");
